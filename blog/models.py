...
class BaseModel(models.Model):
    ...
    #         self,
    #         Post) and 'update_fields' in kwargs and kwargs['update_fields'] == ['views']
    #     if is_update_views:
...
class Category(BaseModel):
    ...
    """
    name = models.CharField("分类名", max_length=40, unique=True, blank=False)
...
class Post(BaseModel):
    ...
    body = MDTextField() # models.TextField("正文")

    # 这两个列分别表示文章的创建时间和最后一次修改时间，存储时间的字段用 DateTimeField 类型。
    # 已过期，已建立BaseModel，包含这两个字段，新建模型继承即可
    # created_time = models.DateTimeField("创建时间", default=timezone.now)
    ...
    pub_time = models.DateTimeField(
        '发布时间', blank=False, null=False, default=timezone.now)

    # 点赞和评论时，记着更新like_count，comment_count。自增1/自减1
    ...
    # 记得从 django.urls 中导入 reverse 函数 todo
    def get_absolute_url(self):
...
class TreeHole(BaseModel):
    ...
    """多级评论需要 自关联"""
    parent = models.ForeignKey("TreeHole", null=True, blank=True, on_delete=models.CASCADE)
